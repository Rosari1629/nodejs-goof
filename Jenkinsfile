pipeline {
    ...
    stages {
        ...
        stage('SAST SonarQube') {
            ...
            steps {
                catchError(buildResult: 'SUCCESS', stageResult: 'FAILURE') {
                    sh 'sonar-scanner -Dsonar.projectKey=nodejsgoof -Dsonar.qualitygate.wait=true -Dsonar.sources=. -Dsonar.host.url=http://192.168.1.18:9000 -Dsonar.token=sqp_b03915ed17a0a99c83ba3d8db968d6e57832ddbe' 
                }
            }
        }
        ...
        stage('Deploy Docker Image') {
            ...
            steps {
                withCredentials([sshUserPrivateKey(credentialsId: "DeploymentSSHKey", keyFileVariable: 'keyfile')]) {
                    ...
                    sh 'ssh -i ${keyfile} -o StrictHostKeyChecking=no fadly@192.168.1.18 docker pull fadly31/nodejsgoof'
                    sh 'ssh -i ${keyfile} -o StrictHostKeyChecking=no fadly@192.168.1.18 docker rm --force nodejsgoof'
                    sh 'ssh -i ${keyfile} -o StrictHostKeyChecking=no fadly@192.168.1.18 docker run -it --detach -p 3001:3001 --name nodejsgoof --network host fadly31/nodejsgoof'
                }
            }
        }
   }
